...
def insert_algolia():
    try:
        ...
        main_logger.info("Storage database connection successful")
        cursor = conn.cursor()
        cursor.execute(
            """
                SELECT 
        ...
        main_logger.info("Agolia connection successful")
            
        main_logger.info('Deleting existing main index...')
        client.delete_index('main_index')

        main_logger.info('Creating new main index...')
        main_index = client.init_index('main_index')

        main_logger.info('Processing data in batches')
        ...
        main_logger.info('Indexing complete. New main index created with updated data.')

        cursor.close()
        conn.close()
    except Exception as e:
        main_logger.error(f"Error when inserting the data to Agolia Search: {e}")
        raise e    